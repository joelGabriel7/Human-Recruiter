                ...
                with transaction.atomic():
                    ...
                    for i in json.loads(request.POST['headings']):
                        ...
                        for key, value in heading.items():
                            ...
                            detail.save()
                        salary_detail.income = salary_detail.salaryheadings_set.filter(headings__type='remuneracion').aggregate(result=Coalesce(Sum('valor',),0.00,  output_field=DecimalField())).get('result')
                        salary_detail.expenses = salary_detail.salaryheadings_set.filter(headings__type='descuentos').aggregate(result=Coalesce(Sum('valor',default=0.00),0.00,  output_field=DecimalField())).get('result')
